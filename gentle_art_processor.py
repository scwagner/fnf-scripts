...
def process_row(row, retry=0):
    ...
    if 'sku' in row:
        ...
        if 'existingPicture' in row and len(row['existingPicture']) > 0:
            time.sleep(1)
            upload_image(object_id, row['imageSlug'], f"gentle-art/{row['existingPicture']}", row['flossName'])
...
def main():
    ...
    try:
        number_processed = 0
        number_failed = 0
        with open('gentle-art.csv', 'r', newline='') as csvfile:
        ...
        print("Error: gentle-art.csv file not found")
