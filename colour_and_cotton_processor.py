...
def upload_image(object_id, image_slug, local_filename, item_name, retry=0):
    print(f"Uploading image {local_filename} to {object_id} with slug {image_slug}")

    # make sure the file exists
    if not os.path.exists(local_filename):
        print(f"Error: {local_filename} does not exist")
        return None
    ...
    # If we got a 429, sleep for 1 second and try again. Retry a max of 3 times
    if response.status_code == 429:
        if retry < 3:
            time.sleep(1)
            return upload_image(object_id, image_slug, local_filename, item_name, retry + 1)
        else:
...
def process_row(row, retry=0):
    ...
    template = read_json_template("colour-and-cotton-floss-template.json")
    ...
    if 'sku' in row:
        ...
        if response.status_code == 429:
            if retry < 3:
                time.sleep(1)
                return process_row(row, retry + 1)
            else:
                print(f"Error: {response.json()}")
                return None
        ...
        if 'existingPicture' in row and len(row['existingPicture']) > 0:
            time.sleep(1)
            upload_image(object_id, row['imageSlug'], f"colour-and-cotton/{row['existingPicture']}", row['flossName'])
...
def main():
    ...
    try:
        number_processed = 0
        number_failed = 0
        with open('colour-and-cotton.csv', 'r', newline='') as csvfile:
        ...
        print("Error: colour-and-cotton.csv file not found")
